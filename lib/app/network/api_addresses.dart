class ApiAddresses{
  static const String baseUrl = 'https://pannel.unique-tech.ir';
  static const String getAccounts = '/accountant/api/getallaccounts';
  static const String addAccount = '/accountant/api/addnewaccount';
  static const String deposit = '/accountant/api/plus';
  static const String withDraw = '/accountant/api/mines';
  static const String getTransactions = '/accountant/api/getalltransactions';

  static const String login = '/account/login/';
  static const String verifyOtp = '/account/login/';
  static const String register = '/account/register/';
  static const String refreshToken = '/token/refresh/';
}